"""Tool schema builder for Gemini function declarations."""
...
def trim_desc(text: str, max_len: int) -> str:
...
def build_gemini_tools(manifests: list[dict], reasoning_mode: str) -> list[types.Tool] | None:
    """Convert skill manifests into Google GenAI types.Tool list."""
    if not manifests:
        return None
    fn_declarations = []
    core_fast_tools = {
    ...
    for td in manifests:
        ...
        for pname, pinfo in properties_source.items():
            ...
            prop: dict[str, Any] = {
                "type": mapped,
                "description": trim_desc(raw_pdesc, param_desc_limit),
            }
            if pinfo.get("enum"):
        ...
        fn_declarations.append(
            types.FunctionDeclaration(
                name=td["name"],
                description=trim_desc(td["description"], tool_desc_limit),
                parameters=schema,
            )
        )
