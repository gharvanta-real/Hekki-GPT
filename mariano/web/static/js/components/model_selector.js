...
const MODES = [
  { id: 'fast', title: 'Gemini 3.1 Flash Lite', desc: 'Active Model', icon: googleIcon }
];

export async function updateModelPills() {
  try {
    const label = 'Gemini 3.1 Flash Lite';
...
export function bindModelPills() {
  document.querySelectorAll('.model-pill').forEach(btn => {
    btn.addEventListener('click', (e) => {
      e.stopPropagation();
      // Single model active, no extra dropdown needed
    });
  });
}

/**
 * Register model pill refresh with the router.
 * Now model pills auto-update on every page navigation without hard refresh.
 */
