...
# Ordered core prompt files that compose the master SYSTEM_PROMPT
_CORE_LAYERS: list[str] = [
    "00_identity.md",
    "01_writing_style.md",
    "02_research.md",
    "03_swe_agent.md",
...
def load_system_prompt() -> str:
    """
    Assemble and return the master SYSTEM_PROMPT from all core layers.
    Result is cached after first call — restart to reload edits.
    """
    sections: list[str] = []
    for filename in _CORE_LAYERS:
    ...
    assembled = "\n\n---\n\n".join(sections)
    logger.info("PromptLoader: SYSTEM_PROMPT assembled from %d sections.", len(sections))
    return assembled
...
def load_planner_prompt() -> str:
    ...
    content = _read_md(_PROMPTS_DIR / "planner.md")
    if not content:
...
def load_evaluator_prompt() -> str:
    ...
    content = _read_md(_PROMPTS_DIR / "evaluator.md")
    if not content:
...
def load_native_coder_prompt() -> str:
    ...
    content = _read_md(_PROMPTS_DIR / "native_coder.md")
    if not content:
